...
export function Uan() {
  ...
  React.useEffect(() => {
    if (uanCards.length > 1 && uanCards[0].image && uanCards[1].image) {
      generateA4PdfPreview(uanCards[0].image, uanCards[1].image);
    } else {
      setA4PdfUrl(null);
    }
          ...
          <div className="max-w-6xl mx-auto space-y-6">
            ...
            <div className="w-full flex justify-center">
              <div className="bg-green-900/90 text-green-200 font-semibold rounded-lg px-8 py-4 mb-4 text-center max-w-2xl">
                ...
                <br />
                We only support original UAN PDF files. Please do not upload any other file.
              </div>
            </div>
                    ...
                    <div className="flex flex-col items-center gap-6">
                      ...
                      <div className="w-full aspect-[1.6/1] bg-gray-800 rounded-lg overflow-hidden flex items-center justify-center mb-2 relative">
                        ...
                        <button
                          ...
                          disabled={uanCards.length < 2}
                          aria-label="Previous"
                          style={{ visibility: uanCards.length < 2 ? 'hidden' : 'visible' }}
                        >
                          <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2"><path d="M13 17l-5-5 5-5"/></svg>
                        </button>
                        {/* Card Image */}
                        {uanCards.length > carouselIndex && uanCards[carouselIndex]?.image ? (
                          <img
                            src={uanCards[carouselIndex].image}
                            alt={carouselIndex === 0 ? "UAN Front" : "UAN Back"}
                      ...
                      <div className="flex gap-2 w-full mt-4">
                        <Button
                          onClick={() => handleDownload(uanCards[carouselIndex], carouselIndex)}
                          className={`flex-1 ${carouselIndex === 0 ? "bg-indigo-500 hover:bg-indigo-600" : "bg-purple-500 hover:bg-purple-600"} text-white`}
                          disabled={!(uanCards.length > carouselIndex && uanCards[carouselIndex].image)}
                        >
                          <Download className="h-4 w-4 mr-2" />
                          Download {carouselIndex === 0 ? "Front" : "Back"}
                        </Button>
                        <Button
                          onClick={() => handlePrint(uanCards[carouselIndex], carouselIndex)}
